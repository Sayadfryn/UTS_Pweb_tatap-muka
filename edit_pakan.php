...
if ($_POST) {
    ...
    $stmt = $pdo->prepare("UPDATE pakan SET nama_pakan = ?, jenis = ?, stok = ? WHERE id = ?");
    if ($stmt->execute([$nama_pakan, $jenis, $stok, $_GET['id']])) {
        header("Location: pakan.php?msg=Pakan berhasil diperbarui");
        exit;
    } else {
        $error = "Gagal memperbarui pakan!";
    }
}
?>
<!DOCTYPE html>
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 card animate-fade-in max-w-2xl mx-auto">
            ...
            <form method="POST">
                <div class="mb-4">
                    ...
                    <input type="text" name="nama_pakan" required value="<?php echo htmlspecialchars($pakan['nama_pakan']); ?>" 
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="jenis" required value="<?php echo htmlspecialchars($pakan['jenis']); ?>" 
                           class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-600">
                </div>
                <div class="mb-4">
                    <label class="block text-gray-700 text-sm font-bold mb-2"><i class="fas fa-weight mr-2"></i>Stok (kg)</label>
                    <input type="number" name="stok" required min="0" value="<?php echo htmlspecialchars($pakan['stok']); ?>" 
                           class="w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-600">
                </div>
